...
export default function CertificatesPage() {
  // Current user mock ID is "u2" - Volunteer John
  const myCertificates = MOCK_CERTIFICATES.filter(c => c.recipientName === "Volunteer John");
              ...
              <Card key={cert.id} className="overflow-hidden border-border/40 shadow-sm">
                <CardContent className="p-6 flex flex-col sm:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <p className="text-xs text-muted-foreground mt-2 font-mono">CERT ID: {cert.id.toUpperCase()}-HOPE-2026</p>
                  </div>
                      ...
                      <DialogContent className="sm:max-w-lg">
                        ...
                        <div className="aspect-[1.4/1] bg-surface-container-low border-8 border-double border-border rounded-lg flex items-center justify-center p-8 text-center relative overflow-hidden">
                          ...
                          <div className="relative z-10 space-y-4">
                            ...
                            <p className="text-xs text-muted-foreground pt-4">Date: {new Date(cert.date).toLocaleDateString()}</p>
                          </div>
                        </div>
                      </DialogContent>
                    </Dialog>
